...
class CarLights {
  ...
  constructor(webgl: any, options: any, colors: any, speed: any, fade: any) {
    this.webgl = webgl; this.options = options; this.colors = colors;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
class HyperspeedApp {
  options: any; container: HTMLDivElement; hasValidSize: boolean;
  renderer: THREE.WebGLRenderer; composer: EffectComposer;
  camera: THREE.PerspectiveCamera; scene: THREE.Scene;
  fogUniforms: any; clock: THREE.Clock; disposed: boolean;
  road: Road; leftCarLights: CarLights; rightCarLights: CarLights;
  leftSticks: LightsSticks; fovTarget: number; speedUpTarget: number;
  speedUp: number; timeOffset: number;
  ...
  setSize(w: number, h: number, u: boolean) {
    if (w <= 0 || h <= 0) { this.hasValidSize = false; return; }
    this.composer.setSize(w, h, u);
    this.hasValidSize = true;
  }
  tick() {
...
export default function Hyperspeed({ effectOptions }: { effectOptions?: EffectOptions }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (merged as any).distortion = (distortions as any)[merged.distortion as string] || distortions.turbulentDistortion;
